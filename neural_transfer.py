...
STYLE_SCALE = 1.0
ITERATIONS = 1000
VGG_PATH = 'imagenet-vgg-verydeep-19.mat' # fichier contenant les poids du reseaux VGG19 devant etre place dans le workpath 
POOLING = 'max'

def build_parser():                       # differentes options de la commande appelant le script
...
def main():
    ...
    if not os.path.isfile(options.network):
        parser.error("Le reseaux %s n'existe pas. (il faut le telecharger...)" % options.network)
    ...
    if options.checkpoint_output and "%s" not in options.checkpoint_output:
        parser.error("Pour sauvegarder les images intermediaires, le parametre checkpoint output doit contenir `%s` (e.g. `foo%s.jpg`)")
    ...
    for iteration, image in stylize(
        ...
        checkpoint_iterations=options.checkpoint_iterations): # beaucoup d'arguments
