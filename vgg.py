...
def load_net(data_path):
    ...
    if not all(i in data for i in ('layers', 'classes', 'normalization')):
        raise ValueError("Vous utilisez les mauvaises donnes VGG19.")
...
def net_preloaded(weights, input_image, pooling):
    ...
    for i, name in enumerate(VGG19_LAYERS):
        ...
        if kind == 'conv':
            ...
            # matconvnet: les poids sont [width, height, in_channels, out_channels]
            # tensorflow: les poids sont [height, width, in_channels, out_channels]
